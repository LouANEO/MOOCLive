...
merging = function(folder)
{
	...
	YearInSecond = year[match(data$Year,year[,1]),2] * 24 * 60 *60
	MonthInSecond = month[cbind(as.integer(data$Month),year[match(data$Year,year[,1]),3])] * 24 * 60 * 60

	s = sort.int(as.integer(data$Second) + as.integer(data$Minute) * 60+ as.integer(data$Hour) * 60 * 60+ as.integer(data$Day) * 60 *60 * 24+ MonthInSecond+ YearInSecond,index.return=TRUE)[2]$ix

	# Write the data in a file

	write.table(data[s,],paste(folder,"/Extraction/data.txt",sep = ""),sep="\t",col.names=TRUE,row.names=FALSE)
}
